...
| `SWE_ROLLOUT_RETRIES` | `2` | Maximum number of *additional* retries for transient **rollout** failures; `2` = up to 3 total attempts. Use `0` to disable rollout retries. This covers the whole rollout attempt — sandbox boot, workspace prep, the agent `run()`, `git_diff`, and sandbox teardown — not just setup: a retryable failure (classified by `_is_retryable`) from any of these is retried up to this budget. Independent of `SWE_EVAL_MAX_ATTEMPTS`: this guards the agent rollout, that guards the fresh evaluator sandbox. How far into a rollout a retryable failure may be retried — in particular once a turn has been recorded for the session — is governed by `SWE_ROLLOUT_RETRY_POLICY`. |
...
| `SWE_AGENT_RETRY_MIN_BUDGET_SEC` | half of `SWE_AGENT_TIME_BUDGET_SEC` | Minimum agent time budget that must remain before a retry is started. If a failure happens late enough that less than this many seconds of the agent budget remain, the rollout fails instead of retrying into a run that cannot finish. Set to `0` to disable the check. Applies to all retry policies. |
